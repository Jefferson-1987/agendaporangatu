...
def update_table(start, end, clinic, admit_type, heatmap_click, reset_click, *args):
    ...
    if heatmap_click is not None and prop_id != "reset-btn":
        weekday = heatmap_click["points"][0]["y"]  # Dia da semana clicado (ex: "Monday").
        hour_of_day = heatmap_click["points"][0]["x"]  # Hora clicada.

        # Determina os dias da semana correspondentes ao clique:
        # Mapeia os dias para números (ex: "Monday" = 0, "Tuesday" = 1, etc.)
        weekday_number = day_list.index(weekday)

        # Filtra dados da semana (segunda a domingo correspondente ao clique).
        clicked_df = filtered_df[
            (filtered_df["Days of Wk"].apply(lambda x: day_list.index(x)).between(
                weekday_number - 3, weekday_number + 3
            ))  # Considera 3 dias antes e depois
        ]
